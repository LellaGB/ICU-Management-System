...
//validation
 qryQuery.SQL.Text := 'SELECT * FROM tblMedicine where [Medicine Code] =' +sDelete;
  qryQuery.Open;
 // iCode := qryQuery.RecordCount;
 if qryQuery.RecordCount = 0 then
 begin
 bFlag := true;
   Showmessage ('This medicine code does not exist within the data base, either update the medicine data base or try a different medicine code');
   exit;
 end;

   { while not qryQuery.Eof do
    begin
    if (Medicine Code).Value = sDelete then
    begin
...
begin
rDate := dtpMedicine.Date;
sUpdate := Inputbox('Medicine Code',' Please enter the code of the medicine you wish to update','');
